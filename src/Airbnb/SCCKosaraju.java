...
public class SCCKosaraju {
  ...
  public static List<List<Integer>> scc(List<Integer>[] graph) {
    ...
    //get the post order of the original Graph
    ...
    //the index of the components is the id of one SCC
    List<List<Integer>> components = new ArrayList<>();
    Arrays.fill(used, false);
    //reverse post order elements
    Collections.reverse(order);
    //Doing the dfs in reverse post order to reversed graph
    for (int u : order)
      if (!used[u]) {
